...
function preload()
{
    img = loadImage('Ventilado.jpeg');
}

function draw()
{
    image(img, 0, 0, 220, 420);
    ...
    if(stats != "")
    {
        for (i = 0; i < objects.length; i++)
        {
            ...
            text(objects[i].label + " " + porcentagem + "%", objects[i].x, objects[i].y + 20);
            noFill();
            stroke('red');
            rect(objects[i].x, objects[i].y + 20, objects[i].width + 100, objects[i].height + 100);
        }
    }
}
